...
if (cwdPackageJsonConfig.dll) {
  ...
  const dllPath = path.resolve(paths.appPublic, cwdPackageJsonConfig.basename || '', 'dll.app.js');
  if (!fs.existsSync(dllPath)) {
    console.log(
      chalk.red(`${dllPath} is not existed.\r\n${chalk.cyan('Please run: npm run build-dll')}`)
    );
    process.exit();
  }
}
...
if (!cwdPackageJsonConfig.dll) {
  ...
  entry.push(require.resolve(util.pathResolve('config/polyfills.js', scriptsPackagename)));
}
//webpack配置项
var config = {
  //任何错误立即终止
